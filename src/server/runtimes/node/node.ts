...
// Helper function to generate native test wrapper
function generateNativeTestWrapper(
  ...
  return `// Native test wrapper generated by testeranto
// Hash: ${filesHash}
// Framework: ${framework}
...
if ('${framework}' === 'jest') {
  // Jest tests need to be run via jest runner
  // For now, we'll import and execute the original test
  import('${originalTestAbs}').then(() => {
    console.log('Jest test imported successfully');
  }).catch(error => {
    console.error('Failed to run Jest test:', error);
    process.exit(1);
  });
} else if ('${framework}' === 'mocha') {
  // Mocha tests
  import('${originalTestAbs}').then(() => {
    console.log('Mocha test imported successfully');
  }).catch(error => {
    console.error('Failed to run Mocha test:', error);
    process.exit(1);
  });
} else if ('${framework}' === 'vitest') {
  // Vitest tests
  import('${originalTestAbs}').then(() => {
    console.log('Vitest test imported successfully');
  }).catch(error => {
    console.error('Failed to run Vitest test:', error);
    process.exit(1);
  });
} else {
...
async function startBundling(
  ...
  // Process each entry point to detect native tests
  ...
  for (const entryPoint of entryPoints) {
    ...
    if (fs.existsSync(entryPointPath)) {
      ...
      entryPointInfo.set(entryPoint, detectionResult);
      
      if (detectionResult.isNativeTest) {
        console.log(`[NODE BUILDER] Detected native ${detectionResult.frameworkType} test: ${entryPoint}`);
      }
    }
  }
