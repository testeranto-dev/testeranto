...
export async function updateEntrypointForServiceStart(
  ...
  if (existingNode) {
    ...
    const currentNeeds = metadata.needsServiceStart ? 
      (Array.isArray(metadata.needsServiceStart) ? metadata.needsServiceStart : [metadata.needsServiceStart]) 
      : [];
    
    if (!currentNeeds.includes(serviceType)) {
      currentNeeds.push(serviceType);
    }
    ...
    await graphManager.applyUpdate({
      operations: [{
        ...
        data: {
          ...
          metadata: {
            ...metadata,
            needsServiceStart: currentNeeds,
