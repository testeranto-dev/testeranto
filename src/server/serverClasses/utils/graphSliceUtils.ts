...
import { addAgentNodesFromConfigUtil } from "./graphManagerUtils";

export function addAgentNodesFromConfig(
  graphManager: GraphManager,
  configs: ITesterantoConfig
): void {
  addAgentNodesFromConfigUtil(graphManager, configs);
}

export function updateAgentSliceFile(
  graphManager: GraphManager,
  projectRoot: string,
  configs: ITesterantoConfig,
  agentName: string
  ...
  try {
    ...
    const sliceData = agentConfig.sliceFunction(graphManager);
...
export function updateAllAgentSliceFiles(
  graphManager: GraphManager,
  projectRoot: string,
  configs: ITesterantoConfig
): void {
...
export function getViewNodes(graphManager: GraphManager): any[] {
  const graphData = graphManager.getGraphData();
  return graphData.nodes.filter((node: any) => node.type === 'view');
}

export function getViewNode(graphManager: GraphManager, viewKey: string): any {
  const viewNodes = getViewNodes(graphManager);
  return viewNodes.find((node: any) => node.viewKey === viewKey);
}

export function getViewSlice(graphManager: GraphManager, viewKey: string): {
  nodes: any[],
  edges: any[]
} {
  const viewNode = getViewNode(graphManager, viewKey);
  if (!viewNode) {
