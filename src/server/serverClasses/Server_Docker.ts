...
export class Server_Docker extends Server_WS {
  ...
  generateServices() // config: IBuiltConfig,
    : Record<string, any> {
    ...
    for (const [runtimeTestsName, runtimeTests] of Object.entries(
      ...
      if (!hasBuildKitOptions) {
        console.warn(
          `[Server_Docker] Warning: No BuildKit options configured for ${runtimeTestsName}. ` +
      ...
      if (!processedRuntimes.has(runtime)) {
        ...
        console.log(
          `[Server_Docker] Added builder service: ${builderServiceName} for ${runtime}`,
        );
      }
      ...
      for (const tName of testsObj) {
        ...
        console.log(
          `[Server_Docker] [generateServices] ${runtimeTestsName} BDD command: "${bddCommand}"`,
        );
  ...
  async start() {
    ...
    try {
      ...
      console.log(
        `[Server_Docker] Created base reports directory: ${baseReportsDir}`,
      );
    } catch (error: any) {
      console.error(
        `[Server_Docker] Failed to create base reports directory ${baseReportsDir}: ${error.message}`,
      );
    }
    ...
    console.log(
      "[Server_Docker] Starting builder services to create test bundles",
    );
    ...
    if (this.mode === "once") {
      try {
        console.log(
          "[Server_Docker] Once mode: Waiting for tests to complete...",
        );
        await this.waitForAllTestsToComplete();

        console.log(
          "[Server_Docker] Once mode: All tests completed. Exiting now.",
        );
        process.exit(0);
      } catch (error: any) {
  ...
  async watchOutputFile(
    ...
    console.log(
      `[Server_Docker] Setting up output file watcher for: ${outputDir} (configKey: ${configKey}, test: ${testName})`,
    );
    ...
    if (this.mode === "dev") {
      fs.watch(outputDir, (eventType, filename) => {
        if (filename) {
          console.log(
            `[Server_Docker] Output directory changed: ${eventType} ${filename} in ${outputDir}`,
          );
          this.updateOutputFilesList(configKey, testName, outputDir);
  ...
  private updateOutputFilesList(
    ...
    try {
      ...
      console.log(
        `[Server_Docker] Updated output files for ${configKey}/${testName}: ${relativePaths.length} files`,
      );
      ...
      console.error(
        `[Server_Docker] Failed to read output directory ${outputDir}:`,
        error.message,
      );
      this.outputFiles[configKey][testName] = [];
    }
  ...
  private loadInputFileOnce(
    ...
    try {
      ...
      console.warn(
        `[Server_Docker] Could not get input file path for ${runtime}/${configKey}: ${error.message}`,
      );
      return;
    }

    console.log(
      `[Server_Docker] Loading input file once: ${inputFilePath} (configKey: ${configKey})`,
    );
    ...
    if (fs.existsSync(inputFilePath)) {
      try {
        ...
        if (allTestsInfo[testName]) {
          ...
          console.log(
            `[Server_Docker] Loaded ${testInfo.files?.length || 0} input files for ${testName} from ${inputFilePath}, hash: ${testInfo.hash || "none"}`,
          );
        } else {
          console.warn(
        ...
        console.warn(
          `[Server_Docker] Failed to read input file ${inputFilePath}: ${error.message}`,
        );
      }
    } else {
      console.log(
  ...
  async watchInputFile(runtime: IRunTime, testsName: string) {
    ...
    console.log(
      `[Server_Docker] 👀 Setting up file watcher for: ${inputFilePath} (configKey: ${configKey})`,
    );
    ...
    if (fs.existsSync(inputFilePath)) {
      try {
        ...
        if (allTestsInfo[testsName]) {
          ...
          console.log(
            `[Server_Docker] 📖 Loaded ${testInfo.files?.length || 0} input files for ${testsName} from ${inputFilePath}, hash: ${testInfo.hash || "none"}`,
          );
          if (testInfo.files && testInfo.files.length > 0) {
            console.log(
              `[Server_Docker]   Files: ${testInfo.files
                .slice(0, 3)
                .map((f) => path.basename(f))
                .join(", ")}${testInfo.files.length > 3 ? "..." : ""}`,
            );
          }
        } else {
          console.warn(
            `[Server_Docker] ⚠️ Test ${testsName} not found in ${inputFilePath}`,
          );
        ...
        console.warn(
          `[Server_Docker] ⚠️ Failed to read input file ${inputFilePath}: ${error.message}`,
        );
      }
    } else {
      console.log(
        `[Server_Docker] 📭 Input file does not exist yet: ${inputFilePath}`,
      );
      console.log(
        `[Server_Docker]   Tests will be triggered when this file is created.`,
      );
    }
    ...
    if (this.mode === "dev") {
      try {
        fs.watchFile(inputFilePath, (curr, prev) => {
          console.log(
            `[Server_Docker] 🔄 Input file changed: ${inputFilePath}`,
          );
          console.log(
            `[Server_Docker] 📊 Previous mtime: ${prev.mtime}, Current mtime: ${curr.mtime}`,
          );

          // Check if file exists before trying to read it
          if (!fs.existsSync(inputFilePath)) {
            console.log(
              `[Server_Docker] ⚠️ Input file no longer exists: ${inputFilePath}`,
            );
            return;
          }
          ...
          try {
            ...
            if (allTestsInfo[testsName]) {
              ...
              console.log(
                `[Server_Docker] 📄 Updated input files for ${configKey}/${testsName}: ${testInfo.files?.length || 0} files`,
              );
              console.log(
                `[Server_Docker]   Old hash: ${oldHash}, New hash: ${newHash}`,
              );
              ...
              if (newHash !== oldHash) {
                console.log(
                  `[Server_Docker] 🔄 Hash changed for ${testsName}, triggering tests...`,
                );
                // Find the configuration for this runtime and test
                for (const [ck, configValue] of Object.entries(
                  this.configs.runtimes,
                )) {
                  if (
                    configValue.runtime === runtime &&
                    ...
                    console.log(
                      `[Server_Docker] 🚀 Triggering tests for ${runtime}/${testsName} (config: ${ck})`,
                    );
                    ...
                    console.log(
                      `[Server_Docker] ✅ All tests triggered for ${runtime}/${testsName}`,
                    );
                    break;
                  }
                }
                ...
                console.log(
                  `[Server_Docker] ⏭️ Hash unchanged for ${testsName}, skipping test execution`,
                );
              }
            } else {
              console.warn(
            ...
            console.error(
              `[Server_Docker] ❌ Failed to read or parse input file ${inputFilePath}: ${error.message}`,
            );
          }
        });
      } catch (e: any) {
        console.error(
          `[Server_Docker] ❌ Failed to watch file ${inputFilePath}: ${e.message}`,
        );
      }
    } else {
  ...
  async informAider(
    ...
    console.log(
      `[Server_Docker] Informing aider service: ${aiderServiceName} about updated input files`,
    );
    ...
    try {
      ...
      if (!containerId) {
        console.error(
          `[Server_Docker] No container found for aider service: ${aiderServiceName}`,
        );
        return;
      }

      console.log(
        `[Server_Docker] Found container ID: ${containerId} for ${aiderServiceName}`,
      );
      ...
      try {
        ...
        console.log(
          `[Server_Docker] Read input files from ${inputFilesPath}, length: ${inputContent.length}`,
        );
      } catch (error: any) {
        console.error(
          `[Server_Docker] Failed to read input files: ${error.message}`,
        );
      ...
      console.log(
        `[Server_Docker] Executing command to send input to aider process`,
      );
      ...
      try {
        ...
        console.error(
          `[Server_Docker] Failed to send input via docker exec: ${error.message}`,
        );
      }
    } catch (error: any) {
      console.error(
      ...
      this.captureExistingLogs(aiderServiceName, runtime).catch((err) =>
        console.error(`[Server_Docker] Also failed to capture logs:`, err),
      );
    }
  }
  ...
  async launchBddTest(
    ...
    console.log(
      `[Server_Docker]   Config: ${configKey}, Test: ${testName}, Runtime: ${runtime}`,
    );
    ...
    try {
      ...
      this.startServiceLogging(bddServiceName, runtime).catch((error) =>
        console.error(
          `[Server_Docker] ❌ Failed to start logging for ${bddServiceName}:`,
          error,
        ),
      );
      ...
      console.log(
        `[Server_Docker] ✅ BDD test launched successfully: ${bddServiceName}`,
      );
    } catch (error: any) {
      console.error(
        `[Server_Docker] ❌ Failed to start ${bddServiceName}: ${error.message}`,
      );
      // Even if starting failed, try to capture any logs that might exist
      this.captureExistingLogs(bddServiceName, runtime).catch((err) =>
        console.error(`[Server_Docker] Also failed to capture logs:`, err),
      );
  ...
  async launchChecks(
    ...
    for (let i = 0; i < checks.length; i++) {
      ...
      console.log(
        `[Server_Docker] Starting check service: ${checkServiceName}`,
      );
      try {
        await spawnPromise(
        ...
        this.captureExistingLogs(checkServiceName, runtime).catch((error) =>
          console.error(
            `[Server_Docker] Failed to capture existing logs for ${checkServiceName}:`,
            error,
          ),
        );

        // Then start logging fresh logs
        this.startServiceLogging(checkServiceName, runtime).catch((error) =>
          console.error(
            `[Server_Docker] Failed to start logging for ${checkServiceName}:`,
            error,
          ),
        );
        ...
        console.error(
          `[Server_Docker] Failed to start ${checkServiceName}: ${error.message}`,
        );
        // Try to capture logs even if starting failed
        this.captureExistingLogs(checkServiceName, runtime).catch((err) =>
          console.error(`[Server_Docker] Also failed to capture logs:`, err),
        );
      }
    }
  ...
  private async captureExistingLogs(
    serviceName: string,
    runtime: string,
  ): Promise<void> {
  ...
  public async stop(): Promise<void> {
    ...
    for (const [containerId, logProcess] of this.logProcesses.entries()) {
      try {
        ...
        console.error(
          `[Server_Docker] Error stopping log process for ${containerId}:`,
          error,
        );
      }
    }
    ...
    console.log(
      `[Server_Docker] Docker down result: ${result.exitCode === 0 ? "success" : "failed"}`,
    );
  ...
  private writeConfigForExtension(): void {
    try {
      ...
      if (this.configs.runtimes && typeof this.configs.runtimes === "object") {
        for (const [key, value] of Object.entries(this.configs.runtimes)) {
          ...
          if (runtimeObj && typeof runtimeObj === "object") {
            ...
            console.log(
              `[Server_Docker] Found runtime: ${runtime}, tests:`,
              tests,
            );
            ...
            if (runtime) {
              ...
              console.warn(
                `[Server_Docker] No runtime property found for key: ${key}`,
                runtimeObj,
              );
            }
          } else {
            console.warn(
              `[Server_Docker] Invalid runtime configuration for key: ${key}, value type: ${typeof value}`,
            );
          }
        }
      } else {
      ...
      console.log(
        `[Server_Docker] Updated extension config with ${processSummary.total || 0} processes`,
      );
    } catch (error: any) {
  ...
  public getInputFiles(runtime: string, testName: string): string[] {
    console.log(
      `[Server_Docker] getInputFiles called for ${runtime}/${testName}`,
    );
    ...
    console.log(
      `[Server_Docker] Found config key: ${configKey} for ${runtime}/${testName}`,
    );
    ...
    if (
      ...
      console.log(
        `[Server_Docker] Found ${files.length} input files in memory for ${configKey}/${testName}`,
      );
      return Array.isArray(files) ? files : [];
    }

    console.log(
      `[Server_Docker] No input files in memory for ${configKey}/${testName}`,
    );
    console.log(
      `[Server_Docker] Available config keys:`,
      Object.keys(this.inputFiles || {}),
    );
    if (this.inputFiles && this.inputFiles[configKey]) {
      console.log(
        `[Server_Docker] Tests in ${configKey}:`,
  ...
  public getAiderProcesses(): any[] {
    try {
      ...
      const aiderProcesses = summary.processes.filter(
      ...
      console.error(
        `[Server_Docker] Error getting aider processes: ${error.message}`,
      );
      return [];
    }
  }
  ...
  public handleAiderProcesses(): any {
    try {
      ...
      console.error(
        `[Server_Docker] Error in handleAiderProcesses: ${error.message}`,
      );
      return {
        aiderProcesses: [],
        timestamp: new Date().toISOString(),
  ...
  public getProcessSummary(): any {
    try {
      ...
      try {
        ...
        console.error(
          `[Server_Docker] Error running docker ps: ${dockerError.message}`,
        );
        ...
        if (containerId && containerId.trim()) {
          try {
            ...
            if (inspectOutput && inspectOutput !== "") {
              const [exitCodeStr, startedAtStr, finishedAtStr] =
                inspectOutput.split("|");
              if (
                exitCodeStr &&
                exitCodeStr !== "" &&
                exitCodeStr !== "<no value>"
              ) {
            ...
            console.debug(
              `[Server_Docker] Could not inspect container ${containerId}: ${error}`,
            );
          }
        }
      ...
      console.error(
        `[Server_Docker] Unexpected error in getProcessSummary: ${error.message}`,
      );
      return {
        processes: [],
        total: 0,
  ...
  private async startServiceLogging(
    ...
    try {
      ...
      console.error(
        `[Server_Docker] Failed to create report directory ${reportDir}: ${error.message}`,
      );
      return;
    }
    ...
    console.log(
      `[Server_Docker] Starting log capture for ${serviceName} to ${logFilePath}`,
    );
    ...
    try {
      ...
      console.warn(
        `[Server_Docker] Could not get container ID for ${serviceName}, will track by service name`,
      );
    }
    ...
    child.on("error", (error) => {
      console.error(
        `[Server_Docker] Log process error for ${serviceName}:`,
        error,
      );
    ...
    child.on("close", (code) => {
      ...
      logStream.end();
      console.log(
        `[Server_Docker] Log process for ${serviceName} exited with code ${code}`,
      );
  ...
  private async captureContainerExitCode(
    serviceName: string,
    runtime: string,
  ): Promise<void> {
  ...
  private async buildWithBuildKit(): Promise<void> {
    ...
    if (!buildKitAvailable) {
      throw new Error(
        "BuildKit is not available. Please ensure Docker BuildKit is enabled. " +
    ...
    console.log(
      "[Server_Docker] BuildKit is available. Building all runtimes...",
    );
    ...
    try {
      ...
      console.error(
        `[Server_Docker] ❌ Aider image build failed:`,
        error.message,
      );
    ...
    for (const [configKey, configValue] of Object.entries(
      ...
      console.log(
        `[Server_Docker] Building ${runtime} runtime (${configKey})...`,
      );
      ...
      try {
        ...
        switch (runtime) {
          case "node":
            ...
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (node)`,
            );
            break;
          ...
          case "web":
            ...
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (web)`,
            );
            break;

          case "golang":
            console.log(
              `[Server_Docker] Building golang runtime: ${configKey}`,
            );
            await golangBuildKitBuild(this.configs, configKey);
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (golang)`,
            );
            break;
          ...
          case "ruby":
            ...
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (ruby)`,
            );
            break;
          ...
          case "rust":
            ...
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (rust)`,
            );
            break;
          ...
          case "java":
            ...
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (java)`,
            );
            break;

          case "python":
            console.log(
              `[Server_Docker] Building python runtime: ${configKey}`,
            );
            await pythonBuildKitBuild(this.configs, configKey);
            console.log(
              `[Server_Docker] ✅ BuildKit build successful for ${configKey} (python)`,
            );
            break;
        ...
        console.error(
          `[Server_Docker] ❌ BuildKit build failed for ${configKey} (${runtime}):`,
          error.message,
        );
    ...
    if (buildErrors.length > 0) {
      ...
      console.log(
        `[Server_Docker] ✅ All BuildKit builds completed successfully!`,
      );
    }
  }
  ...
  private async startBuilderServices(): Promise<void> {
    ...
    for (const serviceName of builderServices) {
      ...
      try {
        ...
        console.log(
          `[Server_Docker] ✅ Builder service ${serviceName} started successfully`,
        );
        ...
        if (runtimeForService) {
          this.startServiceLogging(serviceName, runtimeForService).catch(
            (error) =>
              console.error(
                `[Server_Docker] Failed to start logging for ${serviceName}:`,
                error,
              ),
          );
        }
...
CMD ["tail", "-f", "/dev/null"]`;
      ...
      try {
        ...
        console.error(
          `[Server_Docker] Failed to create ${dockerfilePath}: ${error.message}`,
        );
        return;
      }
    }
  ...
  private async waitForAllTestsToComplete(): Promise<void> {
    console.log(
      "[Server_Docker] Once mode: Waiting for all tests to complete...",
    );

    // Wait a bit for containers to start
    console.log(
      "[Server_Docker] Waiting 10 seconds for containers to start...",
    );
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      console.log(
        `[Server_Docker] Waiting for ${runningContainers.length} test containers to finish... (attempt ${attempt + 1}/${maxAttempts})`,
      );
  ...
  public async DC_build(): Promise<IDockerComposeResult> {
    throw new Error(
      "Traditional docker-compose build is not supported. Use BuildKit instead.",
    );
  }
}
