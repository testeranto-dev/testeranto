...
export async function processFeatureUrlPure(
  ...
  if (isUrl) {
    if (featureIngestor) {
      try {
        console.log(`[processFeatureUrlPure] Processing feature URL with featureIngestor: ${featureUrl}`);
        ...
        console.error(`[processFeatureUrlPure] Error processing feature URL ${featureUrl}:`, error);
        return { content: `Error processing feature: ${(error as Error).message}` };
      }
    } else {
      console.warn(`[processFeatureUrlPure] Feature URL detected but no featureIngestor provided: ${featureUrl}`);
    ...
    if (fs.existsSync(localPath)) {
      try {
        ...
        console.error(`[processFeatureUrlPure] Error reading local feature file ${localPath}:`, error);
        return { content: `Error reading local file: ${(error as Error).message}` };
      }
    } else {
      console.warn(`[processFeatureUrlPure] Feature file not found: ${featureUrl}`);
