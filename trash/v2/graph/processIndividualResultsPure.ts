  ...
  if (singleTestResult.individualResults.length > 0) {
    for (let i = 0; i < singleTestResult.individualResults.length; i++) {
      ...
      if (individualResult.features && Array.isArray(individualResult.features)) {
        ...
        // console.log(`[processIndividualResultsPure] No features found for individual result ${i}`);
      }
      ...
      if (inputFiles && Array.isArray(inputFiles) && inputFiles.length > 0) {
        if (entrypointId) {
          ...
          console.log(`[GraphManaprocessIndividualResultsPureger] No entrypointId for individual result, skipping`);
        }
      } else {
        console.log(`[processIndividualResultsPure] No input files in individual result ${i}`);
      }
    }
  } else {
    console.log(`[processIndividualResultsPure] individualResults array is empty, creating a single placeholder test node`);
