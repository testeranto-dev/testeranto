...
var TesterantoStakeholderApp = (() => {
  ...
  function projectGraph(graph, config) {
    ...
    for (const node of validNodes) {
      ...
      const x3 = getProjectedValue(nodeWithAttributes, config.xAttribute, config.xType, config.xTransform);
      const y3 = getProjectedValue(nodeWithAttributes, config.yAttribute, config.yType, config.yTransform);
  ...
  function layoutForceWithD3(nodes, edges, options = {}) {
    ...
    const simulation = simulation_default(d3Nodes).force("charge", manyBody_default().strength(strength)).force("link", link_default(d3Links).distance(distance)).force("center", center_default(center.x, center.y)).force("collision", collide_default().radius(20)).stop();
    for (let i = 0; i < iterations; i++) {
      simulation.tick();
    }
    return nodes.map((node, index2) => {
      const d3Node = d3Nodes[index2];
      return {
        ...node,
  ...
  function layoutGrid(nodes, spacing = { x: 50, y: 50 }) {
    ...
    return sortedNodes.map((node, index2) => ({
      ...node,
      screenX: index2 * spacing.x,
      screenY: Math.floor(index2 / 10) * spacing.y
    }));
  }
  ...
  function layoutTree(nodes, edges, rootId, orientation, nodeSeparation, levelSeparation) {
    ...
    return nodes.map((node) => {
      ...
      const index2 = nodesAtDepth.findIndex((n) => n.id === node.id);
      if (isHorizontal) {
        return {
          ...node,
          screenX: depth * levelSep,
          screenY: index2 * nodeSep
        };
      } else {
        return {
          ...node,
          screenX: index2 * nodeSep,
  ...
  function layoutTimeline(nodes, timeAttribute) {
    ...
    })).sort((a2, b) => a2.time - b.time);
    return timeNodes.map(({ node }, index2) => ({
      ...node,
      screenX: index2 * 80,
      screenY: 50
    }));
  ...
  var BaseChart = (props) => {
    ...
    const bounds = (0, import_react8.useMemo)(() => {
      ...
      styledGraph.nodes.forEach((node) => {
        const x3 = node.screenX || node.x * width;
        const y3 = node.screenY || node.y * height;
        minX = Math.min(minX, x3);
        maxX = Math.max(maxX, x3);
        minY = Math.min(minY, y3);
        maxY = Math.max(maxY, y3);
    ...
    const resetCamera = () => {
      ...
      const x3 = width / 2 - centerX * scale;
    ...
    const toScreenCoords = (x3, y3) => {
      return {
        x: x3 * camera.scale + camera.x,
        y: y3 * camera.scale + camera.y
      };
    };
    const renderNodes = () => {
    ...
    const renderEdges = () => {
      ...
      return styledGraph.edges.map((edge, index2) => {
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { children: [
      /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { style: {
          ...
          marginBottom: "20px",
          minHeight: "400px"
        }, children: activeTab === "debug" ? renderDebugView() : renderVisualization({
          data,
          vizType: activeTab,
          onNodeClick,
          onNodeHover,
  ...
  var StakeholderGraphClient = class {
    constructor(onGraphUpdate) {
      ...
      __publicField(this, "isDevelopmentMode");
      this.onGraphUpdate = onGraphUpdate;
      this.isDevelopmentMode = typeof window !== "undefined" && window.location.hostname.includes("localhost") && window.location.protocol.startsWith("http");
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: { padding: "20px", fontFamily: "sans-serif" }, children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: { fontSize: "0.8em", color: "#666", marginTop: "30px", paddingTop: "10px", borderTop: "1px solid #eee" }, children: [
        /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("p", { children: [
          ...
          " | Graph nodes: ",
          data.unifiedGraph?.nodes?.length || 0,
          " | Graph edges: ",
