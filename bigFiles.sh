...
usage() {
    ...
    echo "Finds the top 10 largest .ts, .tsx, .rb, .rs, .java, .py, .go files in the specified directory (recursively)"
...
# Sort by size (numeric, reverse) and take top 10
sort -nr "$TEMP_FILE" | head -10 | while read size path; do
