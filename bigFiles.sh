...
done < <(find "$TARGET_DIR" \( -name "node_modules" -o -name "venv" -o -name "target/package" -o -path "*/lib/rusto/target" -o -path "*/lib/rusto/target/*" -o -path "*/dist/*" -o -path "*/lib/pitono/lib/*" -o -path "*/lib/pitono/build/*" \) -prune -o \
    -type f \( \
    -name "*.ts" -o \
    -name "*.tsx" -o \
    -name "*.rb" -o \
