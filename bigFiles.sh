...
done < <(find "$TARGET_DIR" \( -name "node_modules" -o -name "venv" -o -name "target/package" -o -path "*/lib/rusto/target" -o -path "*/lib/rusto/target/*" -o -path "*/dist/*" -o -path "src/lib/pitono/lib/python3.14/*" -o -path "src/lib/pitono/build/*" \) -prune -o \
    -type f \( \
    -name "*.ts" -o \
    -name "*.tsx" -o \
    -name "*.rb" -o \
